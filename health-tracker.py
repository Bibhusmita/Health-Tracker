global name, gender, age, weight, height, itype, bmi
...
def quarterly_stats(months_list):
...
def choose_input_type():
    global itype
    print("\n\nDo you wish to enter weekly(w), monthly(m), or quarterly(q) data?")
...
def weekly():
    ...
    if check is False:
        print("\nPlease make sure complete data according to the chosen option is present in data.txt")
    ...
    if award == 0:
        print("\nCongratulations! You have got a 7/7 award this week! Keep it up!!!\n")

    else:
        print("\nNo Awards this week, as there are breaks in the schedule! Work hard next time!!!\n")
...
def monthly():
    ...
    if check is False:
        print("\nPlease make sure complete data according to the chosen option is present in data.txt")
        print("\nAdd complete data and restart the program")
        exit()
    ...
    if award:
        ...
        print(f"\nThere is no award this month, since you had no consecutive 7/7 weeks. Work harder next time!!\n")
...
def quarterly():
    ...
    metrics, award, result = quarterly_stats(months_list)
    print_basic()
    print("\n*****************************************\n")
    ...
    print(f"\nYour longest distance: { metrics['max_dist']/1000} km")
    print(f"\nYour shortest distance: { metrics['min_dist']/1000} km")
    print(f"\nYour quarterly average speed: { metrics['avg_speed'] * 18/5} km/h")
    print(f"\nYour quarterly average distance is: { metrics['avg_dist']/ 1000} km")

    if award:
        print(f"\nCongrats! You have got {result} M/M awards for this period. Keep it up!!!\n")
    else:
        print(f"\nNo award for this quarter, since you had no consecutive M/M months. Work harder next time!!!\n")
